    ...
    const updateMouse3D = function () {
			...
			return function updateMouse3D( event ) {
				...
				if ( scope.object.isPerspectiveCamera ) {
					...
					v.set(
					    ( (event.clientX - element.getBoundingClientRect().left) / element.clientWidth ) * 2 - 1,
					    - ( (event.clientY - element.getBoundingClientRect().top) / element.clientHeight ) * 2 + 1,
					    0.5 );
					...
					const distance = - v.dot( scope.object.up ) / v1.dot( scope.object.up )
      ...
      if(state !== STATE.NONE){
        ...
        // disable middle/right mouse key
				// case 1:
